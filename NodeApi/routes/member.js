	...
	if(session!=null){
		connection.query('Select exists(Select * from users where session = "'+session+'") as result',function(err, rows, fields){
			if(err){
				...
				if(rows[0].result===1){
					connection.query('insert into member (identity_number, npwp, level_member) values ("'+req.body.identity_number+'" , "'+req.body.npwp+'" , "'+req.body.level_member+'")', function(err, rows, fields) {
						if (err){
						   console.log(err);
						}else{
	...
	if(session!=null){
		connection.query('Select exists(Select * from users where session = "'+session+'") as result',function(err, rows, fields){
			if(err){
				...
				if(rows[0].result===1){
					connection.query('update member set identity_number="'+req.body.identity_number+'", updated_at="'+req.body.updated_at+'", npwp="'+req.body.npwp+'", total_komisi="'+req.body.total_komisi+'", total_saldo="'+req.body.total_saldo+'" where (id_member="'+req.body.id_member+'")', function(err, rows, fields) {
						if (err){
						   console.log(err);
						}else{
